...
use crate::{
    ...
    symbol::{
        semantic::Symbolic, GenericID, GenericKind, GlobalID,
        LocalGenericParameterID, TraitImplementation,
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        for candidate in &self.error.candidates {
            let candidate_qualified_name =
                self.table.get_qualified_name(*candidate).ok_or(fmt::Error)?;
...
impl Display for WithTable<'_, CyclicDependency> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let symbol_list = self
            ...
            .map(|&symbol| {
                let qualified_name =
                    self.table.get_qualified_name(symbol).ok_or(fmt::Error)?;
        ...
        for participant in &self.error.participants {
            if let Some(participant_span) =
                self.table.get_global(*participant).ok_or(fmt::Error)?.span()
            {
                write!(f, "\n{}", SourceCodeDisplay {
...
/// An entity was exposed to the public interface but it's accessbility is less
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        write!(f, "{}", Message {
            ...
            display: format!(
                ...
                self.error.expected_count,
                generic_kind,
                self.error.supplied_count,
            ),
        })?;
...
pub struct InvalidTypeInOutlivesPredicate {
    /// The span to the predicate that contains the invalid type as its
    /// operand.
...
impl Display for WithTable<'_, UnusedGenericParameterInImplementation> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        let generic_symbol =
            self.table.get_generic(generic_id).ok_or(fmt::Error)?;
...
impl Display
    for WithTable<'_, MismatchedGenericParameterCountInImplementation>
{
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let generic_kind = match self.error.generic_kind {
            GenericKind::Type => "type",
...
impl Display for WithTable<'_, MismatchedImplementationConstantTypeParameter> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        let implementation_member_span = match self
            .error
