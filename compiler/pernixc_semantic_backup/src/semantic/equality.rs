...
impl<S: Model> visitor::Visitor for Visitor<'_, S> {
    ...
    fn visit_type(&mut self, ty: &Type<Self::Model>, _: visitor::Source) -> bool {
        if let Some(terms) = <Type<Self::Model> as Map>::get(self.premise_mapping).get(ty) {
            for term in terms {
    ...
    fn visit_lifetime(&mut self, lifetime: &Lifetime<Self::Model>, _: visitor::Source) -> bool {
        if let Some(terms) = <Lifetime<Self::Model> as Map>::get(self.premise_mapping).get(lifetime)
        {
            for term in terms {
    ...
    fn visit_constant(&mut self, constant: &Constant<Self::Model>, _: visitor::Source) -> bool {
        if let Some(terms) = <Constant<Self::Model> as Map>::get(self.premise_mapping).get(constant)
        {
            for term in terms {
