...
impl<S: Model> unification::Config<Constant<S>>
    for TraitResolvingUnifingConfig
{
    ...
    for TraitResolvingUnifingConfig
{
    fn unifiable(&mut self, lhs: &Lifetime<S>, rhs: &Lifetime<S>) -> bool {
        lhs.is_parameter()
            || if self.symetric { rhs.is_parameter() } else { false }
    }
}
...
pub enum Error {
    ...
    InvalidID,
    #[error(
        "implementation resolution requires generic arguments to be definite"
    )]
...
impl<T: State> Table<T> {
    /// Resolves for the implementation of the given trait with the given
    /// generic arguments.
    ///
    /// # Errors
    ///
...
impl GenericArguments<Symbolic> {
    /// In terms of speciality, compares the given generic arguments with the
    /// current one and returns the order.
    #[must_use]
