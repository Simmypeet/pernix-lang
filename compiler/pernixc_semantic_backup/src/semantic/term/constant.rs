...
use super::{
    lifetime::Lifetime, r#type::Type, GenericArguments, MemberSymbol, Model,
    Symbol, Term, TupleElement, Unpacked,
};
use crate::{
    arena::ID,
    semantic::{
        model::Entity, predicate::Premises, unification::Substructural,
    },
...
/// Represents a trait associated constant, denoted by
/// `trait<args>::constant<args>` syntax.
...
/// syntax.
pub type Tuple<S> =
    super::Tuple<Constant<S>, ConstantParameterID, TraitMember<S>>;
...
impl<S: Model> TryFrom<Constant<S>> for Tuple<S> {
    ...
    fn try_from(value: Constant<S>) -> Result<Self, Self::Error> {
        value.into_tuple()
    }
}
...
impl<S: Model> Entity for Constant<S> {
        ...
        S::ScopedLifetime: Into<T::ScopedLifetime>,
    {
        match self {
            ...
            Self::Local(local) => {
                Constant::Local(Local(Box::new(local.0.into_other_model())))
            }
            ...
            Self::Struct(struct_constant) => Constant::Struct(Struct {
                ...
                generic_arguments: struct_constant
                    .generic_arguments
                    .into_other_model(),
            ...
            Self::Enum(enum_constant) => Constant::Enum(Enum {
                ...
                generic_arguments: enum_constant
                    .generic_arguments
                    .into_other_model(),
            ...
            Self::TraitMember(trait_member) => {
                Constant::TraitMember(TraitMember {
                    id: trait_member.id,
            ...
            Self::Implementation(symbol) => {
                Constant::Implementation(MemberSymbol {
                    id: symbol.id,
        ...
        S::ScopedLifetime: TryInto<T::ScopedLifetime>,
    {
        match self {
            ...
            Self::Inference(inference) => {
                inference.try_into().ok().map(Constant::Inference)
            }
            ...
            Self::Enum(enum_constant) => Some(Constant::Enum(Enum {
                ...
                generic_arguments: enum_constant
                    .generic_arguments
                    .try_into_other_model()?,
                associated_value: if let Some(variant) =
            ...
            Self::TraitMember(trait_member) => {
                Some(Constant::TraitMember(TraitMember {
                    id: trait_member.id,
                    parent_generic_arguments: trait_member
                        .parent_generic_arguments
                        .try_into_other_model()?,
            ...
            Self::Implementation(symbol) => {
                Some(Constant::Implementation(MemberSymbol {
                    id: symbol.id,
                    parent_generic_arguments: symbol
                        .parent_generic_arguments
                        .try_into_other_model()?,
