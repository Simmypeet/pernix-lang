...
#[derive(
    Clone, Debug, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, EnumAsInner,
)]
...
impl<S: Model> Entity for Lifetime<S> {
        ...
        S::ScopedLifetime: TryInto<T::ScopedLifetime>,
    {
        match self {
            ...
            Self::Scoped(lifetime) => {
                lifetime.try_into().ok().map(Lifetime::Scoped)
            }
            Self::Inference(lifetime) => {
                lifetime.try_into().ok().map(Lifetime::Inference)
            }
        }
    }
}
