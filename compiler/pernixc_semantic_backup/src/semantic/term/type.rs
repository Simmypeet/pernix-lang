...
use super::{
    constant::Constant, lifetime::Lifetime, MemberSymbol, Model, Symbol, Term, TupleElement,
    Unpacked,
};
use crate::{
    arena::ID,
    ...
    symbol::{self, Enum, GenericID, GlobalID, Struct, TraitType, TypeParameterID},
};
...
impl<M: Model> Term for Type<M> {
    ...
    fn is_adt_type_term(&self) -> bool {
        match self {
            Self::Symbol(symbol) => {
                matches!(symbol.id, SymbolKindID::Struct(_) | SymbolKindID::Enum(_))
            }
            _ => false,
        }
    }
    ...
    fn constant_type_premises(
        ...
        fn filter_constant_type_predicates<M: Model>(k: &NonEquality<M>) -> Option<&Type<M>> {
...
impl<S: Model> Entity for Type<S> {
        ...
        S::ScopedLifetime: Into<T::ScopedLifetime>,
    {
        match self {
            ...
            Self::Implementation(implementation) => Type::Implementation(MemberSymbol {
        ...
        S::ScopedLifetime: TryInto<T::ScopedLifetime>,
    {
        match self {
            ...
            Self::Implementation(implementation) => Some(Type::Implementation(MemberSymbol {
                id: implementation.id,
                member_generic_arguments: implementation
                    .member_generic_arguments
