...
impl<S: Model> unification::Config<Lifetime<S>> for DeductionUnifingConfig {
    fn unifiable(&mut self, lhs: &Lifetime<S>, _: &Lifetime<S>) -> bool {
        lhs.is_parameter()
    }
}
...
impl<M: Model> GenericArguments<M> {
    ...
    ///
    /// This operation performs type deduction on the generic arguments of the
    /// current generic arguments with respect to the generic arguments of
    /// the `another` generic arguments. This deduction will return a
    /// [`Substitution`] that maps the type, lifetime, and constant
    /// parameter of `self` to the type, lifetime, and constant parameter of
    /// `another`.
    ///
    ///
    /// # Examples
