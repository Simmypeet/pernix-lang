    ...
    > visitor::Visitor for ConstantTypeVisitor<'_, '_, '_, Ts, T, S, R>
{
    ...
    fn visit_type(&mut self, ty: &Type<Self::Model>, _: Source) -> bool {
        ...
        let result =
            ConstantType::satisfies(ty, self.premises, self.table, self.semantic, self.session);
...
impl<M: Model> ConstantType<M> {
    ...
    pub fn apply(&mut self, substitution: &Substitution<M>) { self.r#type.apply(substitution); }

    /// Determines whether the `term` can be used as a type of the constant parameter.
    pub fn satisfies<
        T: Term<Model = M>,
        S: Semantic<T>
            + Semantic<Type<<T as Term>::Model>>
...
impl<T: Term + Entity> Outlives<T> {
    ...
    >(
        ...
        let is_applicative = match semantic.outlives_property(term, lifetime_bound) {
        ...
        for (operand, bound) in T::outlives_premises(premises) {
            if operand.equals(term, premises, table, semantic, session)
                && bound.equals(lifetime_bound, premises, table, semantic, session)
            {
        ...
        for (lhs_eq, rhs_eqs) in <T as Map>::get(&premises.mapping) {
            ...
            for rhs_eq in rhs_eqs {
                if Self::satisfies(rhs_eq, lifetime_bound, premises, table, semantic, session) {
                    session.mark_as_done(OutlivesRecord {
                        operand: term,
                        bound: lifetime_bound,
                    });
        ...
        for (lhs_eq, rhs_eqs) in <Lifetime<<T as Term>::Model> as Map>::get(&premises.mapping) {
            if !lhs_eq.equals(lifetime_bound, premises, table, semantic, session) {
                continue;
            }

            for rhs_eq in rhs_eqs {
                if Self::satisfies(term, rhs_eq, premises, table, semantic, session) {
                    session.mark_as_done(OutlivesRecord {
                        operand: term,
                        bound: lifetime_bound,
                    });
        ...
        for normalized in lifetime_bound.normalize(premises, table, semantic, session) {
            if Self::satisfies(term, &normalized, premises, table, semantic, session) {
                session.mark_as_done(OutlivesRecord {
                    operand: term,
                    bound: lifetime_bound,
                });
...
impl<S: Model> unification::Config<Lifetime<S>> for HigherRankedLifetimeUnificationConfig<S> {
    fn unifiable(&mut self, _: &Lifetime<S>, rhs: &Lifetime<S>) -> bool { rhs.is_forall() }
}
...
pub struct TraitSatisfiability<S: Model> {
    /// The additional lifetime constraints that must be satisfied in order to satisfy the trait
    /// predicate.
