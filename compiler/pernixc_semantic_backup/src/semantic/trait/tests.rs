...
fn parse_table(input: impl Display) -> Table {
    ...
    let check = ErrorCheck { has_error: RwLock::new(false) };
    ...
    for error in storage {
        eprintln!("{}", WithTable { table: &result, error: &error });
    }
...
fn trivial_trait_resolution() {
    ...
    let target_implementation_arguments: GenericArguments<Symbolic> =
        GenericArguments {
            lifetimes: Vec::new(),
            types: vec![
    ...
    let mut generic_arguments: GenericArguments<Symbolic> =
        target_implementation_arguments.clone();
...
fn trait_resolution_with_specialization_implementation_one() {
    ...
    let (first_implementation_id, t_type_parameter, u_type_parameter) = 'a: {
        for &implementation in &trait_symbol.implementations {
            ...
            if expected_generic_arguments
                == table.get(implementation).unwrap().signature.arguments
            {
