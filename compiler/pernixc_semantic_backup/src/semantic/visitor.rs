...
pub enum VisitMode<'a, S: State> {
    ...
    /// Additionally visit the types appearing the ADT terms as well (i.e. the
    /// fields of a struct or variant of the enum).
    ///
...
impl<S: State> Clone for VisitMode<'_, S> {
    fn clone(&self) -> Self {
        match self {
            ...
            Self::IncludeAdtStructure(table) => {
                Self::IncludeAdtStructure(table)
            }
        }
    }
...
impl<Term, Parameter, TraitMember> Element
    for Tuple<Term, Parameter, TraitMember>
where
...
impl<S: Model> Type<S> {
    fn accept_internal(
        ...
        match self {
                    ...
                    match adt.id {
                        SymbolKindID::Struct(id) => {
                            ...
                            for field_ty in symbol.fields.values().map(|x| {
                                let mut ty =
                                    x.r#type.clone().into_other_model();
                                ty.apply(
            ...
            Self::Array(array) => {
                array.element.accept(visitor, mode.clone())
                    && array.length.accept(visitor, mode)
            }
...
impl<S: Model> Element for Constant<S> {
    ...
    fn accept(
        ...
        match self {
            ...
            Self::Tuple(tuple) => tuple.accept(visitor, mode),
            Self::Symbol(symbol) => {
                symbol.generic_arguments.accept(visitor, mode)
            }
            Self::Implementation(symbol) => {
                symbol.parent_generic_arguments.accept(visitor, mode.clone())
