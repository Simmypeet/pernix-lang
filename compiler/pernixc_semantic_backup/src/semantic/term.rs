    ...
    + Sync
{
    ...
    type OutlivesPremisesIter<'a>: Iterator<Item = (&'a Self, &'a Lifetime<<Self as Term>::Model>)>
    where
        Self: 'a;
    ...
    fn term_count(&self) -> usize {
        ...
        let _ = self.accept(&mut visitor, visitor::VisitMode::<Success>::OnlySubTerms);
...
impl<S: Model> Visitor for CountVisitor<S> {
    ...
    fn visit_type(&mut self, _: &Type<Self::Model>, _: visitor::Source) -> bool {
        self.count += 1;
        true
    }

    fn visit_lifetime(&mut self, _: &Lifetime<Self::Model>, _: visitor::Source) -> bool {
        self.count += 1;
        true
    }

    fn visit_constant(&mut self, _: &Constant<Self::Model>, _: visitor::Source) -> bool {
        self.count += 1;
        true
    }
}
