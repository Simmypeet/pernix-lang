...
/// Represents a pattern that matches to a specific variant of an enum, denoted
/// by `case Variant(...)` syntax.
...
/// Represents a pattern that might not always match to a value of the given
/// type.
...
/// Represents a pattern that matches multiple tuple elements and packs them
/// into a single tuple value, denoted by `...name` syntax.
#[derive(Debug, Clone, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub struct Packed {
...
pub enum TupleElement<Pattern> {
    /// The pattern will match multiple tuple elements and pack them into a
    /// single tuple value.
    Packed(Packed),
...
pub struct Tuple<Pattern> {
    /// Contains all the patterns that will match to all the elements of the
    /// tuple.
    ///
