...
use pernixc_syntax::syntax_tree::{
    self, item::ModulePath, ConnectedList, GenericIdentifier,
    LifetimeIdentifier, QualifiedIdentifier,
};
...
use crate::{
    arena::{Arena, ID},
    error::{
        self, GenericArgumentCountMismatch, LifetimeExpected,
        LifetimeParameterNotFound, MisorderedGenericArgument, ModuleExpected,
        MoreThanOneUnpackedInTupleType, NoGenericArgumentsRequired,
        ResolutionAmbiguity, SymbolIsNotAccessible, SymbolNotFound,
    ...
    symbol::{
        self, semantic::Symbolic, AdtImplementationMemberID, ConstantParameter,
        Function, Generic, GenericID, GenericKind, Global, GlobalID,
        LifetimeParameter, LifetimeParameterID, MemberID, Module, Trait,
        TraitConstant, TraitFunction, TraitType, TypeParameter,
        TypeParameterID,
    },
};
...
trait GenericParameter: Sized + 'static {
    ...
    fn elision_placeholder<S: Model>(
        config: &mut dyn Config<S>,
    ) -> Option<Self::Argument<S>>;
...
///
/// For example, function declaration declared in a module can be categorized as
/// a non-member generic ID. However, a function declared in a trait is not.
/// Result of a resolution.
///
/// Trait implementation members are included here since we can't refer the
/// trait implementation part (i.e. trait[Args]).
#[derive(
    Debug,
    Clone,
    Copy,
...
pub struct MemberGeneric<S: Model> {
    ...
    ///
    /// if the `id` refers to a ADT implementation member, this generic
    /// arguments is **not yet** deduced.
    pub parent_generic_arguments: GenericArguments<S>,
...
pub trait Config<S: Model> {
    ...
    fn constant_arguments_elision_placeholder(
        &mut self,
    ) -> Option<constant::Constant<S>>;
    ...
    /// Gets notified when a constant is evaluated in a generic constant
    /// arguments.
    ...
    /// Will be called every resolution to a lifetime. Allows the user to
    /// provide an extra source of lifetime.
    ///
...
impl<T: State> Table<T> {
    ...
    pub fn resolve_module_path(
        ...
        // loop through the module path and resolve each module name to a module
        // ID.
            ...
            Resolution::NonMemberGeneric(symbol) => {
                let id = match symbol.id {
                    ...
                    NonMemberGenericID::TraitImplementationType(id) => {
                        return Ok(r#type::Type::Implementation(
                            term::MemberSymbol {
    ...
    fn resolve_root_relative(
        ...
        if let Some(first_attemp) = self.resolve_root_from_using_and_self(
            identifier,
            referring_site,
            handler,
        )? {
    ...
    ///
    /// This function is usally for for resolving a trait in `implements`
    /// declaration.
    ///
    /// # Errors
    ///
            ...
            syntax_tree::r#type::Type::Reference(reference) => {
                let lifetime = reference
                    ...
                    .map_or_else(
                        || {
                            config
                                .lifetime_arguments_elision_placeholder()
                                .map_or_else(
                                    || {
                    ...
                    if element.ellipsis().is_some() {
                        match ty {
                            r#type::Type::TraitMember(ty) => {
                                elements.push(TupleElement::Unpacked(
                                    Unpacked::TraitMember(ty),
                                ));
                            }
                            r#type::Type::Parameter(ty) => {
                                elements.push(TupleElement::Unpacked(
                                    Unpacked::Parameter(ty),
                                ));
                            }
    ...
    /// - [`Error::SemanticError`]: If encountered a fatal semantic error e.g.
    ///   symbol not found.
        ...
        match lifetime_argument.identifier() {
            ...
            LifetimeIdentifier::Identifier(ident) => {
                if let Some(lifetime) =
                    config.extra_lifetime_provider(ident.span.str())
                {
                    return Ok(lifetime);
                }
    ...
    fn get_parent_generic_arguments_from_latest_resolution<S: Model>(
        ...
        latest_resolution.map_or_else(
            ...
            |resolution| {
                resolution.into_non_member_generic().unwrap().generic_arguments
            },
        )
    }
    ...
    pub fn resolve<S: Model>(
        ...
        for generic_identifier in qualified_identifier.generic_identifiers() {
            ...
            // if the symbol resolution is being done in the building state,
            // then the generic parameters will be built from here
            // before the resolution can proceed.
            T::on_global_id_resolved(self, referring_site, global_id);

            config.on_global_id_resolved(
                global_id,
