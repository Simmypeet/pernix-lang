...
use crate::{
    ...
    symbol::{
        Constant, ConstantParameter, ConstantParameterID, Enum, Function,
        Generic, GenericID, GenericKind, GlobalID, Implementation,
        ImplementationConstant, ImplementationFunction, ImplementationType,
        LifetimeParameter, LifetimeParameterID, NegativeImplementation, Struct,
        Symbolic, Trait, TraitConstant, TraitFunction, TraitType, Type,
        TypeParameter, TypeParameterID, Variant,
    },
    table::{
...
impl Table {
        ...
        ID<T>: Into<GenericID> + Into<GlobalID> + Copy,
    {
        ...
        for parameter in
            syntax_tree.into_iter().flat_map(|x| x.parameter_list().elements())
        {
            match parameter {
        ...
        let mut generic_symbol =
            self.get_raw(config.current_id()).unwrap().upgradable_read();
...
impl<T> resolution::Config<Symbolic>
    for AnnonymousLifetimeProvider<'_, '_, '_, T>
where
    ID<T>: Into<GenericID> + Into<GlobalID> + Copy,
    ...
    T: Generic + StatefulSymbol,
{
    ...
    fn type_arguments_placeholder(
        &mut self,
    ) -> Option<crate::entity::r#type::Type<Symbolic>> {
...
implements_finalize!(
    ...
    syntax_tree.signature().generic_parameters().as_ref(),
    {
        ...
        let parent_trait_id =
            table.get(config.current_id()).unwrap().parent_trait_id;
...
implements_finalize!(
    ...
    syntax_tree.signature().generic_parameters().as_ref(),
    {
        // parent generic parameter must be created first
        let parent_impl_id =
            table.get(config.current_id()).unwrap().parent_implementation_id;
        let _ = config.finalize_to(
            parent_impl_id.into(),
            state::FinalizeFlag::GenericParameter,
        );
    ...
    ($kind:ident) => {
        impl Finalize<syntax_tree::item::ImplementationSignature> for $kind {
            fn finalize(
                ...
                let parent_trait_id =
                    table.get(implementation_id).unwrap().signature.trait_id;
                let generic_identifier =
                    syntax_tree.qualified_identifier().rest().last().map_or(
                        syntax_tree.qualified_identifier().first(),
                        |(_, x)| x,
                    );
                let mut annonymous_lifetime_provider =
                    AnnonymousLifetimeProvider { table, config };
                ...
                let generic_argumnent = match table.resolve_generic_arguments(
                    ...
                    Ok(None) => {
                        unreachable!(
                            "the trait symbol is generic, should have generic \
                             arguments"
                        )
                    }
                    Err(_) => {
                        // error, this implementation is invalid, no need to go
                        // further
                        return;
                    }
                };

                table.get_mut(implementation_id).unwrap().signature.arguments =
                    generic_argumnent;
            }
        }
    };
