...
use crate::{
    ...
    entity::{
        ...
        lifetime::Lifetime,
        predicate::{
            self, ConstantType, Equals, Forall, LifetimeOutlives, Predicate, TypeOutlives,
        },
        r#type, Entity, GenericArguments,
    },
    error::{
        self, InvalidTypeInConstantTypePredicate, InvalidTypeInOutlivesPredicate,
        TraitMemberBoundArgumentMismatched, TraitMemberExpected,
    },
    symbol::{
        self, Constant, Enum, Function, Generic, GenericID, GlobalID, Implementation,
        ImplementationConstant, ImplementationFunction, ImplementationType, NegativeImplementation,
        Struct, Symbolic, Trait, TraitConstant, TraitFunction, TraitType, Type, Variant,
    },
    table::{
...
impl resolution::Config<Symbolic> for TraitResolverConfigAdapter<'_, '_> {
    ...
    fn constant_arguments_placeholder(&mut self) -> Option<constant::Constant<Symbolic>> {
...
impl Table {
        ...
        ID<T>: Into<GenericID> + Into<GlobalID> + Copy,
    {
        ...
        match (resolution, trait_member_bound.argument()) {
            ...
            (Resolution::TraitType(trait_ty), TraitMemberBoundArgument::Type(ty)) => {
                let Ok(resolve_ty) =
                    self.resolve_type(ty, config.current_id().into(), config, handler)
                else {
                    return;
                };
            ...
            // mismatched type
            (Resolution::TraitType(..) | Resolution::TraitConstant(..), mismatched) => config
                .handler()
            ...
            (resolved, _) => {
                config
                    .handler()
                    .receive(error::Error::TraitMemberExpected(TraitMemberExpected {
                        non_trait_member_span: trait_member_bound.argument().span(),
                        resolved,
                    }));
            }
        }
        ...
        ID<T>: Into<GenericID> + Into<GlobalID> + Copy,
    {
        ...
        for argument_syn in lifetime_bound.arguments().elements() {
            let Ok(argument) =
                self.resolve_lifetime(argument_syn, config.current_id().into(), config, handler)
            else {
                continue;
            };
        ...
        for clause in where_clause.constraint_list().elements() {
            match clause {
                syntax_tree::item::Predicate::TraitMember(trait_member_bound) => {
                    self.create_trait_member_predicate(trait_member_bound, config);
                }
                ...
                syntax_tree::item::Predicate::Lifetime(lifetime_bound) => {
                    self.create_lifetime_outlives_predicate(lifetime_bound, config);
                }
    ...
    ($kind:ident) => {
        impl Finalize<syntax_tree::item::ImplementationSignature> for $kind {
            fn finalize(
                ...
                table.create_where_clause_predicates(syntax_tree.where_clause().as_ref(), config);

                #[allow(clippy::significant_drop_in_scrutinee)]
                for (checking, _) in config.checkings().iter() {
                    // automatically add lifetime outlives predicates
                    let Checking::Predicate(
                        outlives @ (Predicate::TypeOutlives(_) | Predicate::LifetimeOutlives(_)),
                    ) = checking
                    else {
                        continue;
                    };
