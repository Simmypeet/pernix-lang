...
//!
//! Finalizing symbols occurs when the symbol is being used i.e.resolved, in the
//! code. The symbol must be finalized before it can be used in order to to make
//! it semantically valid. We must also check for cyclic dependency while
//! finalizing the symbol.
...
use crate::{
    ...
    entity::{
        ...
        predicate::{
            ...
            Premises, TypeOutlives,
        },
        r#type, Entity, GenericArguments, Model, Substitution,
    },
    error::{
        self, AmbiguousImplementation, CyclicDependency,
        GenericParameterDuplication, InvalidTypeInConstantTypePredicate,
    ...
    symbol::{
        self, Constant, ConstantParameter, ConstantParameterID, Enum, Function,
        Generic, GenericID, GenericKind, GlobalID, ImplementationKindID,
        ImplementationMemberID, ImplementationSignature, LifetimeParameter,
        LifetimeParameterID, LocalGenericParameterID, Struct, Symbolic, Trait,
        TraitMemberID, Type, TypeParameter, TypeParameterID,
    },
...
/// A struct which composed of the required checking and the span where the
/// checking occurs.
...
/// Is a struct implementing [`Config`] trait that stores the checking and the
/// span where the checking occurs.
...
impl Table {
    ...
    fn check(
        ...
        let premises = Premises::from_predicates(
            premises.into_iter().map(|x| x.predicate),
        );
    ...
    fn create_trait_member_predicate<
        T: Into<GenericID> + Into<GlobalID> + Copy,
    >(
        &self,
        generic_id: T,
        ...
        <Self as Index<T>>::Output: Generic,
    {
        ...
        let Ok(resolution) =
            self.resolve(&operand_syn, generic_id.into(), config, handler)
        else {
            return;
        };
    ...
    // create a lifetime outlives predicate for the given generic symbol
    fn create_reigon_outlives_predicate<
        T: Into<GenericID> + Into<GlobalID> + Copy,
    >(
        &self,
        generic_id: T,
    ...
    fn create_constant_type_predicate<
        T: Into<GenericID> + Into<GlobalID> + Copy,
    >(
        &self,
        generic_id: T,
    ...
    #[allow(clippy::too_many_lines, clippy::significant_drop_tightening)]
    fn create_where_clause_predicates<
        T: Into<GenericID> + Into<GlobalID> + Copy,
    >(
        &self,
        generic_id: T,
        ...
        for clause in where_clause.dissolve().2.into_elements() {
            match clause {
                ...
                syntax_tree::item::Predicate::Lifetime(lifetime_bound) => self
                    .create_reigon_outlives_predicate(
                        generic_id,
                        lifetime_bound,
                        config,
                        handler,
        ...
        <Self as Index<T>>::Output: Generic,
    {
        ...
        let mut generic_symbol =
            self.get_raw(generic_id).unwrap().upgradable_read();
        ...
        insert_generic_parameter!(
            ...
            parameter.0.span(),
            {
                ...
                config.check(
                    Checking::Predicate(Predicate::ConstantType(
                        ConstantType { r#type: constant_type.clone() },
                    )),
                    parameter.1.span(),
                );
        ...
        let starting_index =
            generic_symbol.generic_declaration().parameters.constants.len()
                - default_constant_syns.len();

        for (idx, default_constant) in
            default_constant_syns.into_iter().enumerate()
        {
    ...
    fn finalize_enum(
        ...
        let mut storage = Storage { checkings: &mut checking_with_spans };
    ...
    fn finalize_type(
        ...
        if let Ok(ty) =
            self.resolve_type(&ty_syn, type_id.into(), &mut storage, handler)
        {
            // no private type in public interface
            if self.get_accessibility(type_id.into()).expect("should be valid")
                > self
                    .get_type_overall_accessibility(&ty)
                    .expect("should be valid")
            {
    ...
    fn finalize_trait(
        ...
        let (.., generic_parameters_syn, where_clause_syn) =
        ...
        {
            let mut trait_symbol =
                self.get_raw(trait_id).unwrap().upgradable_read();

            // constructs all the implementations
            trait_symbol.implementations.par_iter().copied().for_each(
                |implementation_id| {
                    println!(
                        "finalizing implementation: {implementation_id:?}"
                    );
                    let _ =
                        self.finalize(implementation_id.into(), None, handler);
                },
            );
            trait_symbol.negative_implementations.par_iter().copied().for_each(
            ...
            let mut valid_implementation_ids: Vec<ImplementationKindID> =
                Vec::new();
            ...
            'outer: for candidate in candidate_implementations {
                for &existing in &valid_implementation_ids {
                    ...
                    let order = existing_generic_argument
                        .order(&candidate_generic_argument);
                    drop((
                        existing_generic_argument,
                        candidate_generic_argument,
                    ));
                    ...
                    match order {
                        ...
                        crate::logic::r#trait::Order::Ambiguous => {
                            handler.receive(
                                error::Error::AmbiguousImplementation(
            ...
            for implementation_id in valid_implementation_ids {
                match implementation_id {
                    ImplementationKindID::Positive(id) => {
                        trait_symbol
                            .with_upgraded(|x| x.implementations.push(id));
                    }
                    ImplementationKindID::Negative(id) => {
                        trait_symbol.with_upgraded(|x| {
                            x.negative_implementations.push(id)
                        });
                    }
                }
            }
    ...
    fn finalize_implementation(
        ...
        let parent_trait_id =
            self.get(implementation_id).unwrap().signature.trait_id;
        let (
            _,
            generic_arguments_syn,
            _,
            qualified_identifier_syn,
            where_clause_syn,
        ...
        let generic_argumnent = match self.resolve_generic_arguments(
            ...
            Ok(None) => unreachable!(
                "the trait symbol is generic, should have generic arguments"
            ),
            Err(_) => {
        ...
        self.get_mut(implementation_id).unwrap().signature.arguments =
            generic_argumnent.clone();
    ...
    /// Checks if all generic parameters defined in the implementation occurs in
    /// the trait arguments.
    ///
    /// # Returns
    ///
    /// Returns `false` if not all generic parameters defined in the
    /// implementation occurs in the trait arguments.
    fn check_implementation_generic_parameters_occurances(
        ...
        let all_lifetime_supplied =
            implementation_signature.arguments.lifetimes.len()
        ...
        let all_constant_supplied =
            implementation_signature.arguments.constants.len()
                == trait_symbol.generic_declaration.parameters.constants.len();

        drop(trait_symbol);

        // make sure that this implementation is valid.
        if !all_lifetime_supplied
            || !all_type_supplied
            || !all_constant_supplied
        {
            println!(
                "{all_lifetime_supplied} {all_type_supplied} \
                 {all_constant_supplied}"
            );
            return false;
        }
    ...
    fn finalize_negative_implementation(
        ...
        let parent_trait_id =
            self.get(negative_implementation_id).unwrap().signature.trait_id;
        ...
        self.get_mut(negative_implementation_id).unwrap().signature.arguments =
            generic_argumnent.clone();
        ...
        self.check_implementation_exlusitivity(
            &self.get(negative_implementation_id).unwrap().signature.arguments,
            negative_implementation_id.into(),
            parent_trait_id,
            handler,
        );
    ...
    fn check_implementation_type_compatibility(
        ...
        // TODO: check if the where clause of the implementation is **strictly**
        // identical to the trait.
        ...
        <Self as Index<U>>::Output: Generic,
    {
        ...
        let parent_generic_parameters =
            &trait_member.generic_declaration().parameters;
        ...
        trait_to_implementation_generic_parameter!(
            Lifetime, Lifetime, Named, lifetimes
        );
        trait_to_implementation_generic_parameter!(
            Type,
            r#type::Type,
        ...
        for (
            idx,
            (trait_constant_parameter, implementation_constant_parameter),
        ) in trait_member
            .generic_declaration()
            .parameters
            .constants
            ...
            .enumerate()
        {
            ...
            if !ty_copied.equals(
                &implementation_constant_parameter.r#type,
                &premises,
                self,
            ) {
                handler.receive(
                    error::Error::MismatchedImplementationConstantTypeParameter(
                        MismatchedImplementationConstantTypeParameter {
    ...
    fn finalize_implementation_type(
        ...
        if self
            ...
            .expect("should be valid")
            > self.get_type_overall_accessibility(&ty).expect("should be valid")
        {
    ...
    fn finalize_variant(
        ...
        if let Some(association) = association {
            ...
            self.get_mut(variant_id).unwrap().associated_type =
                Some(association_type);
        }
    ...
    pub(super) fn finalize(
        ...
        if let Some(referring_site) = referring_site {
            if referring_site == global_id {
                handler.receive(error::Error::CyclicDependency(
                    CyclicDependency { participants: vec![global_id] },
                ));
            ...
            while let Some(dependant) =
                state_manager.dependencies.get(&current_node).copied()
            {
            ...
            state_manager.with_upgraded(|x| {
                x.dependencies.insert(referring_site, global_id)
            });
        }
        ...
        if let Some(referring_site) = referring_site {
            self.state_manager.write().dependencies.remove(&referring_site);
        }
                ...
                for element in &ty.elements {
                    match element {
                        r#type::TupleElement::Regular(reg) => {
                            self.type_occurs(reg)
                        }
                        r#type::TupleElement::Unpacked(
                            r#type::Unpacked::Parameter(param),
                        ) => {
                            self.type_parameters.insert(*param);
                        }
                        r#type::TupleElement::Unpacked(
                            r#type::Unpacked::TraitMember(..),
                        ) => {}
                    }
                }
            }
        ...
        match constants {
            ...
            constant::Constant::Tuple(tuple) => {
                for element in &tuple.elements {
                    match element {
                        constant::TupleElement::Regular(reg) => {
                            self.constant_occurs(reg)
                        }
                        constant::TupleElement::Unpacked(
                            constant::Unpacked::Parameter(param),
                        ) => {
                            self.constant_parameters.insert(*param);
                        }
                        constant::TupleElement::Unpacked(
                            constant::Unpacked::TraitMember(..),
                        ) => {}
                    }
                }
            }
