...
use crate::{
    ...
    symbol::{
        self, Accessibility, Constant, Enum, Function, GenericDeclaration,
        GlobalID, ImplementationSignature, Module, ModuleMemberID, Struct,
        Trait, TraitConstant, TraitFunction, TraitMemberID, TraitType, Type,
        Variant,
    },
    table::Index,
};
...
pub(super) struct Context<'a, 'b> {
    ...
    pub(super) usings_by_module_id:
        RwLock<HashMap<ID<Module>, Vec<syntax_tree::item::Using>>>,
        ...
        Self: IndexMut<ParentID>,
    {
        ...
        match map.entry(name) {
            Entry::Occupied(entry) => {
                handler.receive(error::Error::GlobalRedefinition(
                    GlobalRedefinition {
    ...
    pub(super) fn draft_implementation(
        ...
        match kind {
            ...
            syntax_tree::item::ImplementationKind::Positive(body) => {
                    ...
                    let member_id = match member {
                        syntax_tree::item::ImplementationMember::Type(syn) => {
                            ImplementationMemberID::Type(
                                self.draft_implementation_type(
                                    syn,
                                    implementation_id,
                                    builder,
                                ),
                        ...
                        syntax_tree::item::ImplementationMember::Function(
                            syn,
                        ) => ImplementationMemberID::Function(
                            self.draft_implementation_function(
                                syn,
                                implementation_id,
                                builder,
                            ),
                        ),
                        syntax_tree::item::ImplementationMember::Constant(
                            syn,
                        ) => ImplementationMemberID::Constant(
                            self.draft_implementation_constant(
                                syn,
                                implementation_id,
                                builder,
                            ),
                ...
                let mut implementation_symbol =
                    self.get_mut(implementation_id).unwrap();
...
impl<'a, 'b> Context<'a, 'b> {
        ...
        for trait_member in member_list {
            let trait_member_id = match trait_member {
                syntax_tree::item::TraitMember::Function(syn) => {
                    TraitMemberID::Function(
                        self.draft_trait_function(syn, trait_id),
                    )
                }
                ...
                syntax_tree::item::TraitMember::Constant(syn) => {
                    TraitMemberID::Constant(
                        self.draft_trait_constant(syn, trait_id),
                    )
                }
            };
    ...
    pub(super) fn draft_module(
        ...
        submodule_by_name.into_par_iter().for_each(|(name, submodule)| {
            let submodule_id =
                self.draft_module(submodule, name.clone(), Some(module_id));
        ...
        items.into_par_iter().for_each(|item| {
            ...
            let module_member_id = match item {
                ...
                syntax_tree::item::Item::Function(syn) => {
                    ModuleMemberID::Function(
                        self.draft_function(syn, module_id),
                    )
                }
                ...
                syntax_tree::item::Item::Constant(syn) => {
                    ModuleMemberID::Constant(
                        self.draft_constant(syn, module_id),
                    )
                }
            };
