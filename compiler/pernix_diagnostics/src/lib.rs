...
pub enum HighlightStyle {
    Range(Range<SourcePositionWithoutByteIndex>),
...
///
/// # Arguments
/// * `source_code` - The source code that contains the error
...
pub fn highlight_source_code(
    ...
    match source_position_range.clone() {
        ...
        HighlightStyle::Position(position) => {
            if position.line > source_code.line_number() {
                return;
            }
        }
    }
    ...
    let line_range = {
        let start_line_range = {
            ...
            if !source_code.line(lowered_line_number).unwrap().is_empty() {
                lowered_line_number
            } else {
        ...
        let end_line_range = {
            ...
            let raised_line_number =
                min(source_code.line_number(), original_line_number + 1);
            if !source_code.line(raised_line_number).unwrap().is_empty() {
                raised_line_number
            } else {
                original_line_number
            }
...

    {
        ...
        match source_position_range {
            HighlightStyle::Range(ref range) => {
                println!(
                    "{}{}{}{}{}",
                    bright_cyan.apply_to(source_code.source_name()),
    ...
    for line_number in line_range.clone() {
        ...
        let line = source_code.line(line_number).unwrap();
