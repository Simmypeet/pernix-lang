...
use crate::{database::map, key::DynamicBox, Key};
...
#[test]
fn serializable() {
    let mut serde = super::Serde::default();
    let map = map::Map::default();

    serde.register::<Variable>();
    serde.register::<NegateVariable>();
    ...
    let map = map.serializable(&serde);
...
#[test]
fn incremental_deserializable() {
    // Create a new serde and map with the same reflector setup
    let mut serde = super::Serde::default();
    let map = map::Map::default();
    ...
    // Serialize the map
    let serializable_map = map.serializable(&serde);
    let ron_string =
        ron::to_string(&serializable_map).expect("Failed to serialize to RON");

    // Create a new empty serde and map for deserialization
    let mut target_registry = super::Serde::default();
    ...
    let deserializable_map =
        target_map.incremental_deserializable(&target_registry);
...
fn merge_value_basic_equality() {
    ...
    {
        // Create a separate serde and map with the same value to serialize
        // from
        let mut source_registry = super::Serde::default();
        ...
        let deserializable_map = map.incremental_deserializable(&serde);
        let mut deserializer = ron::Deserializer::from_str(&ron_string)
            .expect("Failed to create deserializer");
    ...
    // merge)
    {
        // Create a separate serde and map with a different value to
        // serialize from
        let mut source_registry = super::Serde::default();
        let source_map = map::Map::default();
        source_registry.register::<Variable>();
...
fn merge_value_conditional_errors() {
    ...
    serde.register::<ConditionalMergeKey>();
...
fn serialize_key() {
    ...
    let mut settings = Settings::clone_current();
    settings.set_sort_maps(true);
    settings.bind(|| assert_ron_snapshot!(key.serializable(&serde)));
}
...
fn deserialize_dynamic_box() {
    ...
    let ron_string = ron::to_string(&original_key.serializable(&serde))
        .expect("Failed to serialize DynamicBox");

    // Deserialize the RON string back to a DynamicBox
    let deserialized_key: DynamicBox = serde
        .dynamic_box_deserializer()
        .deserialize(&mut ron::Deserializer::from_str(&ron_string).unwrap())
        .unwrap();
...
fn deserialize_dynamic_box_different_types() {
    ...
    let variable_ron =
        ron::to_string(&variable_key.serializable(&serde)).unwrap();
    ron::to_string(&variable_key.serializable(&serde))
        .expect("Failed to serialize Variable");
    let deserialized_variable: DynamicBox = serde
        .dynamic_box_deserializer()
        .deserialize(&mut ron::Deserializer::from_str(&variable_ron).unwrap())
        .expect("Failed to deserialize Variable");
    assert_eq!(variable_key, deserialized_variable);

    // Test NegateVariable deserialization
    let negate_ron = ron::to_string(&negate_key.serializable(&serde))
        .expect("Failed to serialize NegateVariable");
    let deserialized_negate: DynamicBox = serde
        .dynamic_box_deserializer()
        .deserialize(&mut ron::Deserializer::from_str(&negate_ron).unwrap())
...
fn serialize_generic_key() {
    ...
    let mut serde = super::Serde::default();
    serde.register::<GenericKey<Variable>>();

    let mut settings = Settings::clone_current();
    settings.set_sort_maps(true);
    settings.bind(|| assert_ron_snapshot!(variable_key.serializable(&serde)));
    ...
    let mut registry2 = super::Serde::default();
    registry2.register::<GenericKey<AdditiveKey>>();

    settings
        .bind(|| assert_ron_snapshot!(additive_key.serializable(&registry2)));
}
