    ...
    fn arbitrary_with(args: Self::Parameters) -> Self::Strategy {
        (
            ...
            .prop_map(|(id, fields)| {
                Self::new(
                    id,
                    fields.into_iter().map(Interned::new_duplicating).collect(),
                )
            })
            .boxed()
    }
...
impl Arbitrary for Enum {
    ...
    fn arbitrary_with(args: Self::Parameters) -> Self::Strategy {
        (
            ...
            .prop_map(|(variant_id, associated_value)| {
                Self::new(
                    variant_id,
                    associated_value.map(Interned::new_duplicating),
                )
            })
            .boxed()
    }
...
impl Arbitrary for Array {
    ...
    fn arbitrary_with(args: Self::Parameters) -> Self::Strategy {
        ...
        .prop_map(|elements| {
            Self::new(
                elements.into_iter().map(Interned::new_duplicating).collect(),
            )
        })
        .boxed()
    }
...
impl Arbitrary for Constant {
    ...
    fn arbitrary_with((): Self::Parameters) -> Self::Strategy {
        let leaf = prop_oneof![
            ...
            proptest::bool::ANY.prop_map(|is_phantom| if is_phantom {
                Self::Phantom
            } else {
                Self::Error(crate::error::Error)
            }),
        ];
        ...
        leaf.prop_recursive(8, 48, 6, move |inner| {
            prop_oneof![
                ...
                2 => Tuple::arbitrary_with(Some(inner)).prop_map(Self::Tuple),
            ]
        })
        .boxed()
    }
