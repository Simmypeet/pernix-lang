...
impl SourceElement for Refutable {
    ...
    fn span(&self) -> RelativeSpan {
        match self {
            ...
            Self::Integer(numeric) => numeric.span,
            Self::Named(named) => named.span,
...
impl NameBindingPoint {
    ...
    pub fn insert(
        ...
        match self.named_patterns_by_name.entry(name.clone()) {
            std::collections::hash_map::Entry::Occupied(entry) => {
                handler.receive(AlreadyBoundName {
                    already_bound_identifier_span: entry.get().span,
                    new_binding_span: name_binding.span,
                    name,
                });
