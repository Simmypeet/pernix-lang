...
use crate::{
    component::derived::generic_parameters::{
        GenericParameters, TypeParameterID,
    },
    table,
    term::{
        constant::Constant,
...
impl<M: Model> table::Display for TraitMember<M>
where
...
impl<M: Model> table::Display for FunctionSignature<M>
where
...
impl<M: Model> table::Display for Array<M>
where
    M::TypeInference: table::Display,
    Constant<M>: table::Display,
    Lifetime<M>: table::Display,
