...
impl Table {
    /// Gets the reflector instance that can be used to serialize all the
    /// derived and input components.
    #[must_use]
    #[allow(clippy::cognitive_complexity, clippy::too_many_lines)]
    pub fn reflector() -> Reflector<GlobalID, ArcTrait, ComponentTag, String> {
        // NOTE: every component must be registered in the reflector in order to
        // be serialized.

        let mut reflector = Reflector::default();

        assert!(reflector
            .register_type::<Accessibility>(ComponentTag::Accessibility));
        ...
        assert!(
            reflector.register_type::<LocationSpan>(ComponentTag::LocationSpan)
        );
        ...
        assert!(reflector.register_type::<GenericParameters>(
            ComponentTag::GenericParameters
        ));
        assert!(
            reflector.register_type::<WhereClause>(ComponentTag::WhereClause)
        );
        assert!(reflector
            .register_type::<Implementation>(ComponentTag::Implementation));
        assert!(reflector.register_type::<TypeAlias>(ComponentTag::TypeAlias));
        assert!(reflector.register_type::<FunctionSignature>(
            ComponentTag::FunctionSignature
        ));
        assert!(reflector.register_type::<ImpliedPredicates>(
            ComponentTag::ImpliedPredicates
        ));
        assert!(reflector
