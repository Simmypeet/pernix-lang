...
proptest! {
    ...
    fn type_specifier_test(
        ...
        prop_assert!(parser.parse_type_specifier(&Dummy).unwrap().into_primitive_type_specifier().is_ok());
        prop_assert!(parser.parse_type_specifier(&Dummy).unwrap().into_qualified_identifier().is_ok());
    }
}
