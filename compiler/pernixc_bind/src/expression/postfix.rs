    ...
    component::{
        derived::{
            ...
            ir::{
                ...
                model::{Constraint, NoConstraint},
                value::{
                    literal::{self, Literal},
                    register::{
                        Assignment, Borrow, Cast, FunctionCall, Load, Register,
                        Variant,
                    },
                    Value,
                },
...
use crate::{
    diagnostic::{
        ...
        InvalidReferenceTypeCasting, MethodNotFound, MismatchedArgumentCount,
        MismatchedQualifierForReferenceOf, SymbolIsNotCallable,
        TooLargeTupleIndex, TupleIndexOutOfBOunds,
        UnexpectedGenericArgumentsInField, UnsafeOperation, UnsafeRequired,
    },
    expression::Target,
    infer::{self, Expected, InferenceVariable, NewTypeErased},
        ...
        'candidate: for trait_id in visible_traits {
            ...
            let trait_generic_arguments = GenericArguments {
                lifetimes: trait_generic_parameter
                    .lifetime_order()
                    .iter()
                    .map(|_| Lifetime::Inference(NewTypeErased))
                    .collect(),
                ...
                trait_function_elided_lifetimes.elided_lifetimes.ids().map(
                    |x| {
                        (
                            ...
                            Lifetime::Inference(NewTypeErased),
                        )
                    },
                ),
            );
