    ...
    component::{
        derived::ir::{
            ...
            instruction::{
                ...
                SwitchJump, SwitchValue, Terminator, UnconditionalJump,
            },
            model::Constraint,
        ...
        input::{Member, Parent, SymbolKind, VariantDeclarationOrder},
    },
...
use crate::{
    ...
    infer::{self, Expected},
    pattern::{get_sub_refutable_from_path, Path},
    Abort, Binder, Error,
};
...
fn get_conditional_value(
    ...
    match refutable {
        ...
        Refutable::Enum(variant) => Some(SwitchValue::Positive(
            table.get::<VariantDeclarationOrder>(variant.variant_id).0 as u64,
        )),
...
impl Binder<'_> {
    ...
    fn handle_match_arms(
        ...
        for arm_state in arm_states {
            let pattern = get_sub_refutable_from_path(
                &arm_infos[arm_state.arm_info_index].reftuable_pattern,
                &main_path,
            )
            .unwrap();

            let Some(value) = get_conditional_value(pattern, self.table) else {
