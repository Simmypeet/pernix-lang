...
fn check_reference_bound_named_pattern(
    ...
    let block = ir
        .control_flow_graph
        .get_block(ir.control_flow_graph.entry_block_id())
        .unwrap();
    ...
    let reference_of_register_id = block
        ...
        .find_map(|instruction| {
            ...
            let Assignment::Borrow(reference_of) = &ir
                .values
                .registers
    ...
    let alloca = ir.values.allocas.get(stored_address_alloca_id).unwrap();
...
fn reference_bound_named() {
    ...
    check_reference_bound_named_pattern(
