    ...
    component::{
        derived::{
            ...
            ir::{
                ...
                model::Constraint,
                pattern::{
                    Boolean, Enum, Integer, Irrefutable, NameBinding,
                    NameBindingPoint, Named, Refutable, Structural, Tuple,
...
pub(super) fn get_sub_refutable_from_path<'a>(
    ...
    match path {
        ...
        Path::Field(field_path) => {
            let structural_pat = get_sub_refutable_from_path(
                refutable,
        ...
        Path::TupleElement(tuple_element_path) => {
            let tuple_pat = get_sub_refutable_from_path(
                refutable,
                &tuple_element_path.tuple_path,
            )?
            .as_tuple()?;

            tuple_pat.elements.get(tuple_element_path.index).map(|x| &x.pattern)
        }

        Path::Variant(variant_path) => {
            let enum_pat = get_sub_refutable_from_path(
                refutable,
