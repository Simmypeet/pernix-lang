//! Contains the definition of the [`TokenStream`] struct
...
impl<ID> Fragment<AbsoluteSpan<ID>> {
    ...
    #[must_use]
    pub const fn start_byte(&self) -> ByteIndex {
        match &self.kind {
            FragmentKind::Delimiter(delimiter) => delimiter.open.token.span.end,
            FragmentKind::Indentation(indentation) => {
                indentation.colon.token.span.end
            }
        }
    }

    /// The byte past the end of the fragment.
    #[must_use]
    #[allow(clippy::missing_const_for_fn)]
    pub fn end_byte(mut self: &Self) -> ByteIndex {
        loop {
            match &self.kind {
                FragmentKind::Delimiter(delimiter) => {
                    return delimiter.close.token.span.end;
                }
                FragmentKind::Indentation(indentation) => {
                    if let Some(x) = self.token_stream.last() {
                        match x {
                            Node::Leaf(leaf) => return leaf.token.span().end,
                            Node::Fragment(fragment) => {
                                self = fragment;
                            }
                        }
                    } else {
                        return indentation.new_line.token.span.end;
                    }
                }
            }
        }
                    ...
                    handler.receive(error::Error::InvalidIndentation(
                        InvalidIndentation {
                            ...
                            expected_indentation: indentation_levels
                                ...
                                .indentation_size,
                            found_indentation: level,
                            previous_indentation_start: indentation_levels
                                .last()
                                .map(|x| x.colon.token.span.clone()),
                        },
                    ));
