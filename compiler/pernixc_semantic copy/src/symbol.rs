...
pub struct WhereClause {
    /// The list of constraints introduced by this where clause
    pub constraints: Constraints,

    /// The span location of this where clause.
...
impl Substitution {
    ...
    /// Transforms this [`Substitution`] into a [`LocalSubstitution`].
    ///
    /// # Returns
    ///
    /// The function returns `Some(LocalSubstitution)` if and only if all the generic parameters are
    /// from the same single [`GenericItem`] and all the generic arguments are complete (no missing
    /// generic arguments in between).
    ///
    /// The function returns `Some(LocalSubstitution::default())` if and only if this substitution
    /// is empty.
    pub fn try_into_local(mut self) -> Option<LocalSubstitution> {
        let Some(targetting_generic_item) = self
            .type_substitutions
            .keys()
            .map(|x| x.generic_item_ref)
            .chain(
                self.constant_substitutions
                    .keys()
                    .map(|x| x.generic_item_ref),
            )
            .chain(
                self.lifetime_substitutions
                    .keys()
                    .map(|x| x.generic_item_ref),
            )
            .next()
            ...
            // empty substitution
            return Some(LocalSubstitution::default());
        };
        ...
        for i in 0..self.lifetime_substitutions.len() {
            let key = LifetimeParameterRef {
                generic_item_ref: targetting_generic_item,
                local_ref: LocalLifetimeParameterRef(i),
            };
            let value = self.lifetime_substitutions.remove(&key).unwrap();
            lifetimes.push(value);
        }

        for i in 0..self.type_substitutions.len() {
            let key = TypeParameterRef {
                generic_item_ref: targetting_generic_item,
                local_ref: LocalTypeParameterRef(i),
            };
            let value = self.type_substitutions.remove(&key).unwrap();
            types.push(value);
        }

        for i in 0..self.constant_substitutions.len() {
            let key = ConstantParameterRef {
                generic_item_ref: targetting_generic_item,
                local_ref: LocalConstantParameterRef(i),
            };
            let value = self.constant_substitutions.remove(&key).unwrap();
            constants.push(value);
        }

        Some(LocalSubstitution {
            lifetimes,
            types,
            constants,
        })
