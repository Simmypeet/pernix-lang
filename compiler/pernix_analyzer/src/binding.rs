    ...
    BoundFunction<'table, 'parser, 'ast>
{
    ...
    /// Bind the given function symbol to a [`BoundFunction`].
    pub fn bind(
        function_symbol: &'table FunctionSymbol<'table, 'parser, 'ast>,
        function_table: &'table FunctionSymbolTable<'table, 'parser, 'ast>,
        type_table: &'table TypeSymbolTable,
...
impl<'table, 'parser: 'ast, 'ast> Binder<'table, 'parser, 'ast> {
    ...
    fn bind_block_scope_statement(
        ...
        for statement in &block_scope_statement.value.statements {
            if let Some(bound_statement) = self.bind_statement(statement) {
    ...
    fn bind_binary_expression(
        &mut self,
        expression: PositionWrapper<&'ast BinaryExpression<'parser>>,
    ) -> Option<BoundExpression<'table, 'parser, 'ast>> {
    ...
    fn bind_literal_expression(
        &mut self,
        expression: PositionWrapper<&'ast LiteralExpression<'parser>>,
    ) -> Option<BoundExpression<'table, 'parser, 'ast>> {
    ...
    fn bind_identifier_expression(
        &mut self,
        expression: PositionWrapper<&'parser IdentifierExpression<'ast>>,
    ) -> Option<BoundExpression<'table, 'parser, 'ast>> {
        match self
            .local_scope_stack
    ...
    fn cast_unchecked(
        bound_expression: BoundExpression<'table, 'parser, 'ast>,
        expression_ast: &'parser PositionWrapper<Expression<'ast>>,
        target_type: &'table TypeSymbol,
    ) -> BoundExpression<'table, 'parser, 'ast> {
        if let BoundExpression::BoundLiteralExpression(literal) =
            &bound_expression
        {
            if let LiteralConstantToken::Number {
    ...
    fn pop_errors(&mut self) -> Vec<Error<'table, 'parser, 'ast>> {
