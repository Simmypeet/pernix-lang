//! This crate is directly copied from the [extfn](https://crates.io/crates/extfn)
use std::mem;
...
fn expand(attr: &TokenStream, input: TokenStream) -> Result<TokenStream> {
    ...
    let extracted_generics =
        extract_impl_generics(&self_type, &mut function.sig.generics);
...
fn move_bounds_to_where_clause(
    ...
    let where_predicates =
        params.iter_mut().filter_map(|generic_param| match generic_param {
...
impl<'gp> FindGenericParam<'gp> {
    pub const fn new(generic_param: &'gp GenericParam) -> Self {
        Self { generic_param, found: false }
    }
}

impl<'ast> Visit<'ast> for FindGenericParam<'_> {
    // lifetime generics
    ...
    fn visit_type_array(&mut self, node: &'ast TypeArray) {
        if let GenericParam::Const(const_param) = self.generic_param {
            self.found |=
                const_param.ident == node.len.to_token_stream().to_string();
        }
    ...
    fn visit_type_path(&mut self, node: &'ast TypePath) {
        if let GenericParam::Type(type_param) = self.generic_param {
            self.found |=
                type_param.ident == node.to_token_stream().to_string();
        }
...
impl<'g> ImplTraitsIntoGenerics<'g> {
    const fn new(generics: &'g mut Generics) -> Self {
        Self { generics, counter: 0 }
    }
}
