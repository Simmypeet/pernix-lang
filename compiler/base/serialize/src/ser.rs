...
where
    S: Serializer<E>,
{
    fn serialize(&self, serializer: &mut S, _: &E) -> Result<(), S::Error> {
...
where
    ...
    S: Serializer<Ext>,
{
    fn serialize(&self, serializer: &mut S, e: &Ext) -> Result<(), S::Error> {
        match self {
    ...
    > Serialize<S, E> for DashSet<T, BH>
{
    fn serialize(
        ...
        serializer.emit_seq(self.len(), e, |mut seq, e| {
            for entry in self.iter() {
                seq.serialize_element(entry.key(), e)?;
            }
...
impl<S: Serializer<E>, E> Serialize<S, E> for AtomicBool {
    fn serialize(
        &self,
        serializer: &mut S,
        _extension: &E,
