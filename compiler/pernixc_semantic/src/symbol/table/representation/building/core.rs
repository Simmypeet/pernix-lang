...
impl<C: Container, S: std::fmt::Debug + Send + Sync> Table<Building<C, S>> {
    ...
    pub(in crate::symbol::table::representation) fn initialize_core(&mut self) {
        ...
        for primitive_ty in Primitive::iter() {
            self.initialize_memcpy_copy_implementation(
                core_module_id,
                copy_trait_id,
        ...
        // the drop function
        {
            ...
            drop_function.insert_parameter(Parameter {
                r#type: Type::Reference(Reference {
                    ...
                    pointee: Box::new(Type::Parameter(MemberID {
    ...
    /// }
    /// ```
    fn initialize_memcpy_copy_implementation(
        &mut self,
    ...
    /// Creates an implementation of the `Copy` trait for the tuple types.
    ///
    /// Synopsis:
    ///
    /// ```txt
