...
use crate::{
    ...
    semantic::{
        ...
        predicate::{self, ConstantType, Equals, LifetimeOutlives, Predicate, TypeOutlives},
        substitution::Substitution,
        term::{self, constant::Constant, lifetime::Lifetime, r#type::Type, GenericArguments},
    },
    symbol::{
        self, semantic::Symbolic, ConstantParameter, ConstantParameterID, Enum, Function, Generic,
        GenericID, GenericKind, GlobalID, ImplementationFunction, ImplementationType,
        LifetimeParameter, LifetimeParameterID, Struct, Trait, TraitFunction, TraitType,
    ...
    ID<T>: Into<GlobalID>,
{
    ...
    fn on_type_resolved(&mut self, ty: &term::r#type::Type<S>, type_span: &Span) {
        self.types
            .entry(self.current_flag)
            .or_default()
            .push(TypeResolutionInfo {
                r#type: ty.clone(),
...
impl Table {
        ...
        <Self as Index<ID<T>>>::Output: Generic,
    {
        ...
        let mut generic_symbol = T::get_arena(self)
            .get(generic_id)
            .unwrap()
            .upgradable_read();
    ...
    /// Gets all the required where clause predicates to make this resolution well-formed.
    ...
    where
        ID<T>: Into<GlobalID>,
    {
        let Some((generic_id, substitution)): Option<(GenericID, Substitution<S>)> =
            (match resolution {
                Resolution::Module(_)
                ...
                Resolution::Struct(id) => Some(Self::build_and_get::<T, Struct, S>(
                    config,
                    id,
                    state::r#struct::Flag::WhereClause,
                )),
                Resolution::Function(id) => Some(Self::build_and_get::<T, Function, S>(
                    config,
                    id,
                ...
                Resolution::TraitFunction(id) => {
                    Some(
                        Self::build_with_parent_and_get::<T, TraitFunction, Trait, S>(
                            config,
                            id,
                ...
                Resolution::Type(id) => Some(Self::build_and_get::<T, symbol::Type, S>(
                    config,
                    id,
        ...
        if let Resolution::Trait(resolution) = resolution {
            predicates.push(Predicate::Trait(crate::semantic::predicate::Trait {
                ...
                generic_symbol
                    ...
                    .push(symbol::Predicate {
                        predicate: predicate::Predicate::TypeEquals(Equals {
                            lhs: term::r#type::Type::TraitMember(term::r#type::TraitMember {
        ...
        <Self as Index<ID<T>>>::Output: Generic,
    {
        ...
        let mut higher_ranked_lifetime_provider = HigherRankedLifetimeProvider {
        ...
        let Ok(resolution) = self
            .resolve(
                syntax_tree.qualified_identifier(),
                generic_id.into(),
                &mut ResolutionConfigChain::new(&mut higher_ranked_lifetime_provider, config),
                handler,
            )
            .map(|x| {
                x.into_trait()
        ...
        <Self as Index<ID<T>>>::Output: Generic,
    {
        ...
        match syntax_tree.operand() {
                    ...
                    generic_symbol
                        ...
                        .push(symbol::Predicate {
                            predicate: predicate::Predicate::LifetimeOutlives(LifetimeOutlives {
                                operand: Lifetime::Parameter(lifetime_parameter_id),
                                argument,
                            }),
                            span: Some(syntax_tree.span()),
                            explicit: true,
            ...
            LifetimeBoundOperand::QualifiedIdentifier(qualified_identifier) => {
                ...
                let ty @ (term::r#type::Type::Parameter(..) | term::r#type::Type::TraitMember(..)) =
                    ty
                else {
        ...
        for clause in where_clause.constraint_list().elements() {
            match clause {
                ...
                syntax_tree::item::Predicate::Trait(trait_bound) => {
                    self.create_trait_bound_predicate(generic_id, trait_bound, config, handler);
                }
                ...
                syntax_tree::item::Predicate::ConstantType(constant_type) => {
                    self.create_constant_type_predicate(generic_id, constant_type, config, handler);
                }
            }
        }
    }
