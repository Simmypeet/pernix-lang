    ...
    fn build(mut config: Config<Self>, data: &mut Self::Data, build_flag: Self::Flag) {
        ...
        match build_flag {
            ...
            Flag::WhereClauseAndCheck => {
                ...
                for resolution in data.resolution.values().flatten().map(|x| &x.resolution) {
                    let predicates = Table::get_required_predicates(&config, resolution);
                    table
                        .get_mut(id)
                        .unwrap()
                        .generic_declaration
                        .predicates
                        .extend(predicates.into_iter().map(|x| symbol::Predicate {
                            predicate: x,
                            span: None,
                            explicit: false,
                        }));
