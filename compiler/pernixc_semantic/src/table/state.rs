...
use crate::{
    arena::ID,
    entity::{constant, lifetime, predicate::Premises, r#type},
    error::{self, CyclicDependency},
    symbol::{
        self, Constant, Enum, Function, GlobalID, Implementation, ImplementationConstant,
        ImplementationFunction, ImplementationType, NegativeImplementation, Struct, Symbolic,
        Trait, TraitConstant, TraitFunction, TraitType, Type, Variant,
    },
};
...
pub(in crate::table) trait StatefulSymbol:
    Sized
    + finalizing::generic_parameter::Finalize<<Self as StatefulSymbol>::SyntaxTree>
    ...
    ID<Self>: Into<GlobalID>,
{
    ...
    fn get_state(state_manager: &super::Manager, id: ID<Self>) -> Option<&State<Self::SyntaxTree>>;
...
impl<SyntaxTree> State<SyntaxTree> {
    pub(super) fn add_checking(&mut self, checking: Checking<Symbolic>, span: Span) {
...
pub(super) struct Manager {
    states_by_enum_id: HashMap<ID<Enum>, State<syntax_tree::item::EnumSignature>>,
    states_by_variant_id: HashMap<ID<symbol::Variant>, State<syntax_tree::item::Variant>>,

    states_by_struct_id: HashMap<ID<Struct>, State<syntax_tree::item::Struct>>,
    states_by_constant_id: HashMap<ID<Constant>, State<syntax_tree::item::Constant>>,
    states_by_type_id: HashMap<ID<Type>, State<syntax_tree::item::Type>>,
    states_by_function_id: HashMap<ID<Function>, State<syntax_tree::item::Function>>,

    states_by_trait_id: HashMap<ID<Trait>, State<syntax_tree::item::TraitSignature>>,
    states_by_trait_function_id:
        HashMap<ID<TraitFunction>, State<syntax_tree::item::TraitFunction>>,
    states_by_trait_type_id: HashMap<ID<TraitType>, State<syntax_tree::item::TraitType>>,
    ...
    states_by_negative_implementation_id:
        HashMap<ID<NegativeImplementation>, State<syntax_tree::item::ImplementationSignature>>,
    states_by_implementation_id:
        HashMap<ID<Implementation>, State<syntax_tree::item::ImplementationSignature>>,

    states_by_implementation_function_id:
        HashMap<ID<ImplementationFunction>, State<syntax_tree::item::ImplementationFunction>>,
    states_by_implementation_type_id:
        HashMap<ID<ImplementationType>, State<syntax_tree::item::ImplementationType>>,
    states_by_implementation_constant_id:
        HashMap<ID<ImplementationConstant>, State<syntax_tree::item::ImplementationConstant>>,
    ...
    ID<T>: Into<GlobalID>,
{
    ...
    pub(super) fn finalize_to(&self, id: GlobalID, to: FinalizeFlag) -> Result<(), FinalizeError> {
    ...
    pub(super) fn checkings(&self) -> MappedRwLockReadGuard<'a, [(Checking<Symbolic>, Span)]>
    where
        T::SyntaxTree: 'static,
    {
...
where
    ...
    ID<T>: Into<GlobalID>,
{
    fn lifetime_arguments_placeholder(&mut self) -> Option<lifetime::Lifetime<Symbolic>> { None }
    ...
    fn on_resolved(&mut self, resolved: &resolution::Resolution<Symbolic>) {
        macro_rules! trait_finalization {
            ($trait_id:expr, $trait_member_id:expr, $trait_generic_arguments:expr, $map:ident) => {
                ...
                let premises =
                    Premises::from_predicates(predicates.into_iter().map(|x| x.predicate));

                // finalize its implementation counter part.
                if let Ok(implementation) =
                    self.table
                        .resolve_implementation(trait_id, trait_generic_arguments, &premises)
                {
                    let implementation = self.table.get(implementation.implementation_id).unwrap();
                    let implementation_member = implementation.$map.get(&$trait_member_id).copied();
                    drop(implementation);
        ...
        match resolved {
            ...
            resolution::Resolution::TraitFunction(resolved) => {
                trait_finalization!(
                    { self.table.get(resolved.member.id).unwrap().parent_trait_id },
                    resolved.member.id,
...
impl Manager {
        ...
        ID<T>: Into<GlobalID>,
    {
        loop {
            ...
            // done building the symbol, wake up all the threads that are waiting for it.
            ...
            match current_flag {
                FinalizeFlag::Drafted => {
                    ...
                    *synchronization.finalize_flag.lock() = FinalizeFlag::GenericParameter;
                }
                FinalizeFlag::GenericParameter => {
                    ...
                    *synchronization.finalize_flag.lock() = FinalizeFlag::WhereClause;
                }
                FinalizeFlag::WhereClause => {
                    // TODO: finalize definition
                    *synchronization.finalize_flag.lock() = FinalizeFlag::Definition;
                }
                FinalizeFlag::Definition => {
                    // TODO: finalize body
        ...
        ID<T>: Into<GlobalID>,
    {
        ...
        // if the symbol is currently being built by another thread, then we wait for it to finish
        // or we will just build it ourselves.

        // none of the threads are building the symbol, so this thread will build it.
        if synchronization
            .building
            .compare_exchange(
                false,
        ...
        ID<T>: Into<GlobalID>,
    {
        ...
        if let Some(referring_site) = referred_from {
            ...
            state_manager_read
                .with_upgraded(|x| x.dependencies.insert(referring_site, (id.into(), to_flag)));
        }

        drop(state_manager_read);

        let result = Self::attemp_finalize_to(state_manager, table, id, to_flag, handler);
...
/*
Local type modifier,
Generic parameter deduction from equality where clause predicate.
