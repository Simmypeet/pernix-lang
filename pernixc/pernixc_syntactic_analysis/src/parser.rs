...
use crate::{
    abstract_syntax_tree::{
        declaration::{
            AccessModifier, ClassDeclaration, ClassFieldDeclaration,
            ClassMemberDeclaration, ClassMethodDeclaration, Declaration,
            NamespaceDeclaration, Parameter, QualifiedTypeAnnotaion,
            TypeAnnotaion, UsingDirective,
        },
        expression::{
            BinaryExpression, BinaryOperator, ClassFiledInitialization,
            ClassInstantiationExpression, Expression, FunctionCallExpression,
            LiteralExpression, MemberFieldAccessExpression,
            QualifiedNameExpression, UnaryExpression, UnaryOperator,
        },
        statement::{
            BlockScopeStatement, IfElseStatement, ReturnStatement, Statement,
            VariableDeclarationStatement, WhileStatement,
        },
        PositionWrapper, PrimitiveTypeUnit, TypeUnit,
    },
...
impl<'src> Parser<'src> {
    ...
    pub fn peek_with_offset(&self, offset: isize) -> Token<'src> {
        let index = (self.current as isize + offset)
            .clamp(0, self.token_stream.len() as isize - 1)
            as usize;
    ...
    fn expect_identifier(&mut self) -> Option<PositionWrapper<&'src str>> {
        ...
        if matches!(ident.token_kind, TokenKind::Identifier) {
            Some(PositionWrapper {
                position: ident.position_range.start.into()
                    ..ident.position_range.end.into(),
                value: ident.lexeme,
            })
        } else {
    ...
    pub fn parse_qualified_name(
        &mut self,
    ) -> Option<PositionWrapper<&'src str>> {
        // move to the next significant token
        self.move_to_significant_token();
        ...
        let qualified_name_string =
            &self.token_stream.source_file().source_code()
                [first_byte_index..last_byte_index];

        Some(PositionWrapper {
            position: first_position.into()
    ...
    pub fn parse_type_unit(
        &mut self,
    ) -> Option<PositionWrapper<TypeUnit<'src>>> {
        let next = self.peek_significant_token();

        let type_unit = match next.token_kind {
            TokenKind::Identifier => {
                TypeUnit::QualifiedName(self.parse_qualified_name()?.value)
            }
            TokenKind::Keyword(Keyword::Bool) => {
                TypeUnit::PrimitiveTypeUnit(PrimitiveTypeUnit::Bool)
            }
            TokenKind::Keyword(Keyword::Void) => {
                TypeUnit::PrimitiveTypeUnit(PrimitiveTypeUnit::Void)
            }
            TokenKind::Keyword(Keyword::Int8) => {
                TypeUnit::PrimitiveTypeUnit(PrimitiveTypeUnit::Int8)
            }
            TokenKind::Keyword(Keyword::Int16) => {
                TypeUnit::PrimitiveTypeUnit(PrimitiveTypeUnit::Int16)
            }
            TokenKind::Keyword(Keyword::Int32) => {
                TypeUnit::PrimitiveTypeUnit(PrimitiveTypeUnit::Int32)
            }
            TokenKind::Keyword(Keyword::Int64) => {
                TypeUnit::PrimitiveTypeUnit(PrimitiveTypeUnit::Int64)
            }
            TokenKind::Keyword(Keyword::Uint8) => {
                TypeUnit::PrimitiveTypeUnit(PrimitiveTypeUnit::Uint8)
            }
            TokenKind::Keyword(Keyword::Uint16) => {
                TypeUnit::PrimitiveTypeUnit(PrimitiveTypeUnit::Uint16)
            }
            TokenKind::Keyword(Keyword::Uint32) => {
                TypeUnit::PrimitiveTypeUnit(PrimitiveTypeUnit::Uint32)
            }
            TokenKind::Keyword(Keyword::Uint64) => {
                TypeUnit::PrimitiveTypeUnit(PrimitiveTypeUnit::Uint64)
            }
            TokenKind::Keyword(Keyword::Float32) => {
                TypeUnit::PrimitiveTypeUnit(PrimitiveTypeUnit::Float32)
            }
            TokenKind::Keyword(Keyword::Float64) => {
                TypeUnit::PrimitiveTypeUnit(PrimitiveTypeUnit::Float64)
            }
            _ => {
        ...
        if let TypeUnit::PrimitiveTypeUnit(_) = type_unit {
            self.next();
        }

        Some(PositionWrapper {
            position: next.position_range.start.into()
                ..next.position_range.start.into(),
            value: type_unit,
        })
    }
    ...
    pub fn parse_type_annotation(
        &mut self,
    ) -> Option<PositionWrapper<TypeAnnotaion<'src>>> {
        let type_unit = self.parse_type_unit()?;
        Some(PositionWrapper {
            position: type_unit.position,
            value: TypeAnnotaion::TypeUnit(type_unit.value),
        })
    }
    ...
    pub fn parse_qualified_type_annotation(
        &mut self,
    ) -> Option<PositionWrapper<QualifiedTypeAnnotaion<'src>>> {
        ...
        Some(PositionWrapper {
            position: first_token.position_range.start.into()
                ..type_annotation.position.end,
            value: QualifiedTypeAnnotaion {
                is_mutable,
                type_annotation,
            },
        })
    ...
    fn parse_top_level_declaration(
        ...
        Vec<PositionWrapper<UsingDirective<'src>>>,
        Vec<PositionWrapper<Declaration<'src>>>,
    ) {
        let condition = |token: Token<'src>| {
            let extra_cond = if is_namespace {
                !matches!(token.token_kind, TokenKind::Punctuator('}'))
            } else {
                true
            };

            extra_cond && !matches!(token.token_kind, TokenKind::EndOfFile)
        };
        ...
        // found (if we are parsing for a namespace)
        while condition(self.peek_significant_token()) {
            if let TokenKind::Keyword(Keyword::Using) =
                self.peek_significant_token().token_kind
            {
    ...
    pub fn parse_declaration(
        &mut self,
    ) -> Option<PositionWrapper<Declaration<'src>>> {
        match self.peek_significant_token().token_kind {
            ...
            _ => self.create_error(SyntacticError::UnexpectedToken {
    ...
    fn parse_access_modifier(
        &mut self,
    ) -> Option<PositionWrapper<AccessModifier>> {
        ...
        let access_mod = match first_token.token_kind {
            ...
            _ => {
                return self.create_error(
                    SyntacticError::AccessModifierExpected {
                        expected_position: first_token
                            .position_range
                            .start
                            .into()
    ...
    fn parse_class_declaration(
        ...
        while !matches!(
            ...
            if matches!(
                ...
                members.push(PositionWrapper {
                    position: access_modifier.position.start.into()
                        ..self.peek().position_range.start.into(),
                    value: ClassMemberDeclaration::ClassFieldDeclaration(
            ...
            // a class method
                ...
                fn parse_parameter<'src>(
                    this: &mut Parser<'src>,
                ) -> Option<PositionWrapper<Parameter<'src>>> {
                    let qualified_type_annotation =
                        this.parse_qualified_type_annotation()?;
                    let identifier = this.expect_identifier()?;

                    Some(PositionWrapper {
                        position: qualified_type_annotation
                            .position
                            .start
                            .into()
                ...
                // list of parameters
                let parameters = self.parse_separated_list::<',', ')', PositionWrapper<Parameter>>(|this| parse_parameter(this))?;
                ...
                let body = match body.value {
                    Statement::BlockScopeStatement(block_scope_statement) => {
                        PositionWrapper {
                ...
                members.push(PositionWrapper {
                    position: access_modifier.position.start.into()
                        ..self.peek().position_range.start.into(),
                    value: ClassMemberDeclaration::ClassMethodDeclaration(
        ...
        Some(PositionWrapper {
            position: first_token.position_range.start.into()
                ..self.peek().position_range.start.into(),
            value: Declaration::ClassDeclaration(ClassDeclaration {
                identifier,
                members,
            }),
        })
    ...
    fn parse_namespace_declaration(
        ...
        let (using_directives, declarations) =
            self.parse_top_level_declaration(true);
    ...
    pub fn parse_using_directive(
        &mut self,
    ) -> Option<PositionWrapper<UsingDirective<'src>>> {
        let first_token = self.peek_significant_token();
        ...
        Some(PositionWrapper {
            position: first_token.position_range.start.into()
                ..self.peek().position_range.start.into(),
            value: UsingDirective { qualified_name },
        })
    }
    ...
    pub fn parse_statement(
        &mut self,
    ) -> Option<PositionWrapper<Statement<'src>>> {
        // move to the next significant token
        match self.peek_significant_token().token_kind {
            ...
            | TokenKind::Keyword(Keyword::Continue) => {
                let token = self.next();
                Some(PositionWrapper {
                    position: token.position_range.start.into()
                        ..token.position_range.end.into(),
            ...
            _ => {
                ...
                Some(PositionWrapper {
                    position: expr.position.start
                        ..self.peek().position_range.start.into(),
                    value: Statement::ExpressionStatement(expr.value),
                })
            }
        }
    }
    ...
    fn parse_return_statement(
        ...
        Some(PositionWrapper {
            position: first_token.position_range.start.into()
                ..self.peek().position_range.start.into(),
            value: Statement::ReturnStatement(ReturnStatement { expression }),
        })
    }
    ...
    pub fn parse_expression(
        &mut self,
    ) -> Option<PositionWrapper<Expression<'src>>> {
    ...
    fn parse_unary_operator(
        ...
        let position = lookahead.position_range.start.into()
            ..self.peek().position_range.start.into();
    ...
    fn parse_class_instantiation_expression(
        ...
        let field_initializations = self.parse_separated_list::<',', '}', PositionWrapper<ClassFiledInitialization>>(|this| {
            let identifier = this.expect_identifier()?;
            this.expect_punctuator('=')?;
            let expression = this.parse_expression()?;

            Some(PositionWrapper {
                position: identifier.position.start.into()
                    ..expression.position.end.into(),
                value: ClassFiledInitialization {
                    identifier,
                    expression,
                },
            })
        ...
        Some(PositionWrapper {
            position: first_token.position_range.start.into()
                ..self.peek().position_range.start.into(),
            value: Expression::ClassInstantiationExpression(
    ...
    fn parse_identifier_expression(
        ...
        if matches!(
            ...
            // parse the arguments
            let arguments = self
                .parse_separated_list::<',', ')', PositionWrapper<Expression<'src>>>(
                    |c| {
                        c.parse_expression()
                    }
                )?;
            ...
            Some(PositionWrapper {
                ...
                value: Expression::QualifiedNameExpression(
    ...
    fn parse_primary_expression(
        ...
        let mut expression = match self.peek().token_kind {
            ...
            TokenKind::LiteralConstant(literal_expression) => {
                ...
                PositionWrapper {
                    position: starting_position.into()
                        ..self.peek().position_range.start.into(),
            ...
            TokenKind::Punctuator('(') => {
                ...
                expression.position.start =
                    left_parenthesis.position_range.start.into();
                expression.position.end =
                    right_parenthesis.position_range.end.into();
            ...
            TokenKind::Keyword(Keyword::New) => {
                self.parse_class_instantiation_expression()?
            }
            ...
            | TokenKind::Punctuator('!') => {
                ...
                PositionWrapper {
                    position: unary_operator.position.start
                        ..expression.position.end,
            ...
            _ => {
                ...
                return self.create_error(SyntacticError::UnexpectedToken {
                    unexpected_position: current_token.start.into()
                        ..current_token.end.into(),
        ...
        loop {
            ...
            if let TokenKind::Punctuator('.') =
                self.next_significant_token().token_kind
            {
                // expect an identifier
                let identifier = self.expect_identifier()?;

                expression = PositionWrapper {
                    position: expression.position.start.into()
                        ..identifier.position.end.into(),
                    value: Expression::MemberFieldAccessExpression(
